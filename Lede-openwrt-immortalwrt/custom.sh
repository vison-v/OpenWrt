...
[ "${repo}" = "lede" ] && sed -i 's/msgstr "诊断"/msgstr "网络诊断"/g' feeds/luci/applications/luci-app-diag-core/po/zh_Hans/diag_core.po
...
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/88/89/g' feeds/luci/applications/luci-app-autoreboot/luasrc/controller/autoreboot.lua
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/msgstr "Socat"/msgstr "端口转发"/g' feeds/luci/applications/luci-app-socat/po/zh_Hans/socat.po
[ "${repo}" = "lede" ] && sed -i 's/89/88/g' feeds/luci/applications/luci-app-filetransfer/luasrc/controller/filetransfer.lua
...
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/KMS 服务器/KMS 服务/g' feeds/luci/applications/luci-app-vlmcsd/po/zh_Hans/vlmcsd.po
...
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/msgstr "IP\/MAC绑定"/msgstr "地址绑定"/g' feeds/luci/applications/luci-app-arpbind/po/zh_Hans/arpbind.po
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && echo -e "\nmsgid \"Rules\"" >> feeds/luci/applications/luci-app-arpbind/po/zh_Hans/arpbind.po
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && echo -e "msgstr \"规则\"" >> feeds/luci/applications/luci-app-arpbind/po/zh_Hans/arpbind.po

[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/44/43/g' feeds/luci/applications/luci-app-usb-printer/luasrc/controller/usb_printer.lua
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/nas/services/g' feeds/luci/applications/luci-app-usb-printer/luasrc/controller/usb_printer.lua
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/NAS/Services/g' feeds/luci/applications/luci-app-usb-printer/luasrc/controller/usb_printer.lua
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/USB 打印服务器/打印服务/g' feeds/luci/applications/luci-app-usb-printer/po/zh_Hans/luci-app-usb-printer.po
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/网络存储/存储/g' feeds/luci/applications/luci-app-usb-printer/po/zh_Hans/luci-app-usb-printer.po

[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/nas/services/g' feeds/luci/applications/luci-app-cifs-mount/luasrc/controller/cifs.lua
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/"挂载 SMB 网络共享"/"挂载 SMB"/g' feeds/luci/applications/luci-app-cifs-mount/po/zh_Hans/cifs.po
...
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/msgstr "ZeroTier"/msgstr "内网穿透"/g' feeds/luci/applications/luci-app-zerotier/po/zh_Hans/zerotier.po
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/services/vpn/g'  feeds/luci/applications/luci-app-zerotier/root/usr/share/luci/menu.d/luci-app-zerotier.json
...
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i '/msgid "SmartDNS"/{n;s/SmartDNS/DNS 加速/;}' feeds/luci/applications/luci-app-smartdns/po/zh_Hans/smartdns.po
[ "${repo}" = "lede" ] || [ "${repo}" = "immortalwrt" ] && sed -i 's/services/vpn/g' feeds/luci/applications/luci-app-smartdns/root/usr/share/luci/menu.d/luci-app-smartdns.json
...
[ "${repo}" = "lede" ] && sed -i 's/解锁网易云灰色歌曲/网易音乐/g' feeds/luci/applications/luci-app-unblockmusic/po/zh_Hans/unblockmusic.po
[ "${repo}" = "lede" ] && sed -i 's/services/vpn/g'  `grep services -rl feeds/luci/applications/luci-app-unblockmusic/luasrc`
