...
{  
    (make -j$(nproc) V=s > make_output.log 2> ${CONFIG_REPO}-${CONFIG_NAME}_make_error.log) || \
    (make -j1 V=s >> make_output.log 2>> ${CONFIG_REPO}-${CONFIG_NAME}_make_error.log)  

    if [ $? -ne 0 ]; then  
        echo "Build failed for ${CONFIG_REPO}-${CONFIG_NAME}!"  
        echo "${CONFIG_REPO}-${CONFIG_NAME}_make_error log:"  
        cat ${CONFIG_REPO}-${CONFIG_NAME}_make_error.log  
        mv -f ${CONFIG_REPO}-${CONFIG_NAME}_make_error.log "${WORKSPACE}" 
